...
**Mitigation:** M4 defaults to 64 MiB upload sessions, 64 KiB chunks copied immediately out of CC:T argument-backed storage, four incomplete uploads per computer/speaker, 64 server-wide, and 60-second expiry. Each declared upload size is reserved before allocation against a separate 256 MiB aggregate budget and released on every cleanup path. Configurable values remain inside non-configurable safety ceilings.
...
**Mitigation:** M4 uses independently configurable file, server LRU, client compressed LRU, transfer, and decoded-playback budgets with coherent defaults. Server pools have fixed 1 GiB ceilings and client pools fixed 512 MiB ceilings. Defaults are 64 MiB files, 512 MiB server content, 256 MiB client content, and 128 MiB each for transfer and decoded playback. Relationship validation ensures one supported maximum-size item can complete each required stage. Never universally full-decode longer content; ring/segment streaming remains later work.
...
**Mitigation:** M4 transfers only on a client cache miss, sends at most once per client/session, defaults to 32 KiB chunks, caps client assembly at eight/128 MiB, cancels replacement/stop/logout/reload state, and only serves a requester still within the default 64-block authoritative audience. The chunk hard ceiling leaves metadata headroom below Minecraft's exact 1 MiB clientbound custom-payload limit. Later long-media delivery still needs measured backpressure.
